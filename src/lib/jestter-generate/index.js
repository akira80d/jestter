  ...
  ast.program.body.forEach((node) => {
    switch (node.type) {
      ...
      case TYPES.ExportDefaultDeclaration:
        nodeTypes.ExportDefaultDeclaration.push(
          exportDefaultDeclarationProcess(node)
        );
        break;
      case TYPES.ExportNamedDeclaration:
        nodeTypes.ExportNamedDeclaration.push(
          exportNamedDeclarationProcess(node)
        );
        break;
...
function makeTestData(nodeTypes, filepath) {
  ...
  const test = makeDefaultTestData(nodeTypes.ExportDefaultDeclaration);
  if (test) {
    testdata.tests.push(test);
    if (test.name) {
      testdata.importset.defaultname = test.name;
    }
  }
...
function makeDefaultTestData(nodes) {
  let test;
  switch (nodes[0].declaration_type) {
    ...
    case TYPES.Identifier:
      ...
      break;
    default:
      throw new Error("makDefaultTestData:default");
      debuglog("mkaeDefaultTestData:default!!!!:", nodes[0].declaration_type);
      debuglog(nodes[0]);
      break;
  }
}
...
function exportDefaultDeclarationProcess(node) {
  let object;
  //FunctionDeclaration/Identifier(App.js)
