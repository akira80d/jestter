...
export default function generate(filepath, argv) {
        ...
        switch (node.type) {
            ...
            case TYPES.VariableDeclarator: {
                ...
                if (node.init.type == "ArrowFunctionExpression") {
                    const test = makeFunctionTestData(node.init, name, ancestors, node.init.type);
    ...
    function returnClassMethods(nodes, argv) {
        const classMethods = { constructor: {}, methods: [] };
        for (let i = 0; i < nodes.length; i++) {
            const node = nodes[i];
     ...
     */
    function callExpressionProcess(node, name, ancestors) {
